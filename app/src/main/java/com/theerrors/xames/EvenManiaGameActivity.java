...
public class EvenManiaGameActivity extends AppCompatActivity {
    ...
    @BindView(R.id.p1_choice) EditText p1_choice;
    @BindView(R.id.p2_choice) EditText p2_choice;
    ...
    public void onPlayer2Input(){
        ...
        if(p2Value.length() >= 1){
            ...
            int predicted_location = moving_thing_y - one_graphBox_size*p2_choice_num;

            if(-predicted_location + moving_thing_x >= boundary_value * one_graphBox_size) {

                TranslateAnimation move = new TranslateAnimation(moving_thing_x + 0,moving_thing_x + 0 , moving_thing_y + 0, moving_thing_y - (boundary_value * p2_choice_num - one_graphBox_size * p2_choice_num));
                move.setDuration(1000);
                move.setFillAfter(true);
                image_moving_thing.startAnimation(move);
                CheckWin();
